...
export default function AvaliarLimpezaPage() {
  ...
  async function handleSalvarOS() {
    ...
    try {
      ...
      setErro(err instanceof ApiError ? err.message : "Nao foi possivel salvar a O.S.");
    } finally {
      setSalvandoOS(false);
    }
  }
  ...
  async function handleAprovar() {
    ...
    try {
      ...
      setErro(err instanceof ApiError ? err.message : "Nao foi possivel aprovar.");
    } finally {
  ...
  async function handleReprovar() {
    ...
    try {
      ...
      setErro(err instanceof ApiError ? err.message : "Nao foi possivel reprovar.");
    } finally {
      setProcessando(false);
    }
  }
  ...
  async function handleNotificar() {
    ...
    try {
      ...
      setErro(err instanceof ApiError ? err.message : "Nao foi possivel enviar notificacao.");
    } finally {
      setProcessando(false);
    }
  }
      ...
      {limpeza && (
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="space-y-6 md:col-span-2">
            ...
            <h2 className="text-lg font-bold text-ink">Checklist e Evidencias</h2>
            <div className="space-y-4">
            ...
            <Card>
              <CardHeader className="border-b border-line bg-surface/50">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Info className="h-4 w-4" /> Acoes do Avaliador
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-5 p-5">
                ...
                {!limpeza.notificacaoEnviada ? (
                  ...
                  <div className="rounded bg-surface p-3 text-center text-sm text-ink/60">
                    O.S. nao pode ser alterada apos notificacao.
                  </div>
                )}
                  ...
                  <div className="space-y-3">
                    {limpeza.notificacaoEnviada ? (
                      <div className="flex flex-col items-center gap-1 rounded-md bg-success/10 p-4 text-center text-success">
                        ...
                        <span className="text-sm font-medium">Notificacao Enviada</span>
                      </div>
                    ) : mostrarNotificar ? (
                      <div className="space-y-2">
                        <Input
                          label="E-mails (virgula)"
                      ...
                      <Button className="w-full" onClick={() => setMostrarNotificar(true)}>
                        <Mail className="mr-2 h-4 w-4" /> Notificar Conclusao
                      </Button>
                    )}
                  </div>
                )}
